...
def area_disp(width, height, offset, theta_max, n):
    ...
    # Define arrays
    arr_theta = np.linspace(0, theta_max, n)
    area = np.zeros(n)
    ...
    for i in range(0, len(arr_theta)):
        ...
        p2 = (
        width * np.cos(theta) + height / 2 * np.sin(theta) - offset, width * np.sin(theta) - height / 2 * np.cos(theta))
        p3 = (
        width * np.cos(theta) - height / 2 * np.sin(theta) - offset, width * np.sin(theta) + height / 2 * np.cos(theta))
...
def dC_dAngle(width, height, offset, theta_max, n, z):
    Curve = area_disp(width, height, offset, theta_max, n)
    Angles = Curve[0]
